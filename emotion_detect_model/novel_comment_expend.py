"""
novel_comment_expend.py
=======================
仿照 data_expend.py，对 novel_comment_data/train.txt 做三阶段数据增强：
  1. 将每条评论拆分为有语义的短句，判断短句情感，加入扩充数据集
  2. 提取每句对情感影响最大的关键词，构建正/负向情感词汇表
  3. 基于词汇表让 DeepSeek 生成 1000 条新句子，加入扩充数据集

支持断点续传：中断后重新运行会从上次进度继续。
仅保存扩充数据（不含原文），与 data_expend.py 行为完全一致。

运行：
    python novel_comment_expend.py

输出：
    novel_comment_data/train_expend.txt   — 扩充数据（仅新增部分）
    novel_comment_vocab.json              — 提取到的情感关键词词汇表
"""
...
TRAIN_FILE    = BASE_DIR / 'novel_comment_data' / 'train.txt'
OUTPUT_FILE   = BASE_DIR / 'novel_comment_data' / 'train_expend.txt'
VOCAB_FILE    = BASE_DIR / 'novel_comment_vocab.json'
PROGRESS_FILE = BASE_DIR / 'novel_comment_expend_progress.json'
...
- 每条长度8~60字
- 风格贴近真实用户（小说/网络文学/书评类评论），多样化，不重复
- 积极关键词参考：{', '.join(pos_sample)}
- 消极关键词参考：{', '.join(neg_sample)}

只输出JSON数组，每项含text和label，示例：
[{{"text": "情节紧凑，非常好看", "label": 1}}, {{"text": "剧情拖沓，弃了", "label": 0}}]"""
...
def generate_sentences(
    ...
    """多线程生成 count 条新评论句子。"""
    per_call = 200
    pos_pool = pos_words if pos_words else ["好看", "精彩", "推荐"]
    neg_pool = neg_words if neg_words else ["拖沓", "失望", "弃了"]
...
def main() -> None:
        ...
        for line in f:
            ...
            if len(parts) == 2:
                try:
                    ...
                    pass
    print(f"共 {len(raw_data)} 条训练数据")

    # 断点续传：加载进度
    prog             = load_progress()
    finished_batches = set(prog.get("finished_batches", []))
    pos_keywords: list[str] = prog.get("pos_keywords", [])
    neg_keywords: list[str] = prog.get("neg_keywords", [])

    # 首次运行清空旧输出
    if not finished_batches and OUTPUT_FILE.exists():
        OUTPUT_FILE.unlink()
        print("  （检测到旧输出文件，已清空）")
    ...
    def handle_batch(b_idx: int) -> None:
        new_samples, pos_kw, neg_kw = process_batch(batches[b_idx])

        with file_lock:
            for text, label in new_samples:
                clean = text.replace('\t', ' ').replace('\n', ' ').strip()
                if clean:
                    out_f.write(f"{clean}\t{label}\n")
            out_f.flush()
    ...
    print(f"\n{'='*52}")
    print(f"扩充完成！")
    print(f"  原始训练集: {len(raw_data)} 条")
    print(f"  扩充数据:   {expanded_count} 条（仅新增，不含原文）")
