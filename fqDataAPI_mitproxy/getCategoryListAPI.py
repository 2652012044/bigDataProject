"""Run getTypeList automation, export captured flows, and extract latest new_category entry."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run automation then export captured flows and extract latest target API entry."
    )
    parser.add_argument(
        "--capture-export-url",
        default="",
        help="Optional override for capture export JSON URL",
    )
    parser.add_argument(
        "--target-host",
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="HTTP timeout when requesting capture export",
    )
...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("Step 0/3: Clear mitmproxy session")
    logger.info("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("Step 2/3: Export mitmproxy JSON")
    logger.info("=" * 70)
    try:
        if args.capture_export_url:
            response = requests.get(args.capture_export_url, timeout=args.timeout)
            response.raise_for_status()
            export_json = response.json()
        else:
            export_json = export_capture_json(args.timeout)
    except Exception as exc:
        logger.error("mitmproxy export failed: %s", exc)
        return 1
    ...
    matched_sorted = sorted(matched, key=get_entry_start_key)
