...
def clear_charles_session(logger, required: bool = False) -> bool:
    """清空 Charles 已有的抓包会话。required=True 时失败直接返回 False。"""
    try:
        ...
        if required:
            logger.error("清空 Charles 抓包失败，终止执行: %s", e)
        else:
...
def export_and_extract_user_info(logger, author_name: str) -> bool:
    """导出 Charles 抓包，提取 /reading/user/basic_info/get/ 的响应体并保存。"""
...
def main(author_name: str) -> bool:
    ...
    try:
        ...
        # 正式开始前再次清空 Charles，确保只抓本次流量
        logger.info("--- 清空 Charles（自动化开始前）---")
        if not clear_charles_session(logger, required=True):
            return False
        logger.info("✓ Charles 已就绪")
